...
/** User stub definition for function: STATUS semGive(SEM_ID) */
EXTERN_C_LINKAGE  STATUS semGive (SEM_ID semId) ;
EXTERN_C_LINKAGE  STATUS CppTest_Stub_semGive (SEM_ID semId) 
{
	if (CppTest_IsCurrentTestCase("TestSuite_SpeedSensor_test_processSpeed_1") ||
		CppTest_IsCurrentTestCase("TestSuite_SpeedSensor_test_sendReports_1")) {
		CPPTEST_JMP(1);
	}
    semGive(semId);
}
