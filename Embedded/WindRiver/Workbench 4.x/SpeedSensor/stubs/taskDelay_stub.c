...
/** User stub definition for function: STATUS taskDelay(_Vx_ticks_t) */
EXTERN_C_LINKAGE  STATUS taskDelay (_Vx_ticks_t ticks) ;
EXTERN_C_LINKAGE  STATUS CppTest_Stub_taskDelay (_Vx_ticks_t ticks) 
{
	if (CppTest_IsCurrentTestCase("TestSuite_SpeedSensor_test_signalData_1")) {
		CPPTEST_JMP(1);
	}
    taskDelay(ticks);
}
