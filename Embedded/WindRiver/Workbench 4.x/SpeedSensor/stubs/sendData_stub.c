...
struct speeding_violation;

int test_sendReports_1_flag = 1;

/** User stub definition for function: bool sendData(SpeedingViolation *) */
EXTERN_C_LINKAGE int sendData (struct speeding_violation * report) ;
EXTERN_C_LINKAGE int CppTest_Stub_sendData (struct speeding_violation * report) 
{
	if (CppTest_IsCurrentTestCase("TestSuite_SpeedSensor_test_sendReports_1") && test_sendReports_1_flag) {
		test_sendReports_1_flag = 0;
		return 0; /* Simulate FAILURE for increased coverage */
	}
    sendData(report);
}
