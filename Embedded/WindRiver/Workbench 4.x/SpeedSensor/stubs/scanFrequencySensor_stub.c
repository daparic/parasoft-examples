...
int test_signalData_1_flag = 1;

/** User stub definition for function: bool scanFrequencySensor(double *) */
EXTERN_C_LINKAGE int scanFrequencySensor (double * frequency) ;
EXTERN_C_LINKAGE int CppTest_Stub_scanFrequencySensor (double * frequency) 
{
	/* Simulate FAILURE on the first iteration */
	if (CppTest_IsCurrentTestCase("TestSuite_SpeedSensor_test_signalData_1") && test_signalData_1_flag) {
		test_signalData_1_flag = 0;
		return 0; 
	}
	return scanFrequencySensor(frequency);
}
